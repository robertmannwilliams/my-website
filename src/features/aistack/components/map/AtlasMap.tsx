...
export function AtlasMap({
  ...
  useEffect(() => {
    ...
    const installMapLayers = () => {
      ...
      map.addSource(PIN_SOURCE_ID, {
        type: "geojson",
        data: buildPinCollection(nodes, stageById, enabledStagesRef.current),
        cluster: true,
        clusterRadius: 40,
    ...
    if (map.isStyleLoaded()) {
      ...
      window.setTimeout(() => onLoadedChangeRef.current(true), 2500);
    }
  ...
  useEffect(() => {
    ...
    if (map && flowLayerReadyRef.current) {
      const source = map.getSource(PIN_SOURCE_ID) as
        | mapboxgl.GeoJSONSource
        | undefined;
      source?.setData(buildPinCollection(nodes, stageById, enabledStages));
      syncHtmlMarkers(
        map,
        markersRef.current,
        nodes,
  ...
  }, [chokepointMode, enabledStages, nodes, stageById]);
  ...
  }, [chokepointMode, enabledStages, nodes]);
...
function syncHtmlMarkers(
  ...
  for (const node of nodes) {
    ...
    const size =
      chokepointMode && isMonopoly ? 38 : chokepointMode && isDuopoly ? 35 : 32;
...
function applyPaperAtlasMapStyle(map: mapboxgl.Map) {
  ...
  for (const layer of map.getStyle().layers ?? []) {
    ...
    if (id.includes("water")) {
      if (layer.type === "fill") {
        setPaint(map, layer.id, "fill-color", MAP_PALETTE.water);
      }
      if (layer.type === "line") {
        setPaint(map, layer.id, "line-color", MAP_PALETTE.water);
      }
